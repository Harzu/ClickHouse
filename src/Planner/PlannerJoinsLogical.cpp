...
void buildJoinUsingCondition(const QueryTreeNodePtr & node, JoinInfoBuildContext & builder_context)
{
    auto operator_function = JoinConditionOperator::Equals;

    auto & using_nodes = node->as<ListNode &>().getNodes();
    size_t num_nodes = using_nodes.size();

    auto & expression_actions = builder_context.expression_actions;
    ...
    for (size_t i = 0; i < num_nodes; ++i)
    {
        ...
        if (join_operator.strictness == JoinStrictness::Asof && i == num_nodes - 1)
            operator_function = JoinConditionOperator::GreaterOrEquals;

        auto op = expression_actions.addFunction(args, operator_function);
...
std::unique_ptr<JoinStepLogical> buildJoinStepLogical(
    ...
    const PlannerContextPtr & planner_context)
{
    JoinInfoBuildContext build_context(join_node, left_header, right_header, planner_context);
