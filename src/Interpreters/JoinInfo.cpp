...
static void serializeJoinActions(WriteBuffer & out, const std::vector<JoinActionRef> & actions, const ActionsDAG * actions_dag)
{
    ...
    std::unordered_map<JoinActionRef::NodeRawPtr, size_t> node_map;
    size_t node_id = 0;
    ...
    for (const auto & action : actions)
    {
        ...
        if (auto it = node_map.find(node); it != node_map.end())
            ...
            throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot find node with id {} in node map for dag {}", node->result_name, actions_dag->dumpDAG());
    }
}
...
static std::vector<JoinActionRef> deserializeJoinActions(ReadBuffer & in, JoinExpressionActions & expression_actions)
{
    ...
    for (size_t i = 0; i < num_actions; ++i)
    {
        ...
        if (auto it = node_map.find(node_id); it != node_map.end())
            actions.emplace_back(it->second, expression_actions);
        else
            throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot find node with id {} in node map for dag {}", node_id, actions_dag->dumpDAG());
    }

    return actions;
}

void JoinInfo::serialize(WriteBuffer & out, const ActionsDAG * actions_dag) const
{
...
JoinInfo JoinInfo::deserialize(ReadBuffer & in, JoinExpressionActions & expression_actions)
{
    ...
    JoinInfo result(kind, strictness, locality);
