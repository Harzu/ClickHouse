...
UInt64 calculateHashFromStep(const JoinStepLogical & join_step)
{
    ...
    for (const auto & condition : join_step.getJoinInfo().expression)
...
void calculateHashTableCacheKeys(QueryPlan::Node & root)
{
    ...
    while (!stack.empty())
    {
        ...
        if (auto * join_step = dynamic_cast<JoinStepLogical *>(node.step.get()))
        {
            ...
            const auto & join_expression = join_step->getJoinInfo().expression;
            bool single_disjunct = join_expression.size() > 1 || (join_expression.size() == 1 && join_expression.front().getFunction() != JoinConditionOperator::Or);
